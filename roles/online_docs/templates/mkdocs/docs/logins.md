...
## SSH config and login to UI via jumphost
 ...
 * Add to your ```${HOME}/.ssh/config``` something like the following:
    ...
    ```# Generic stuff: only for MacOS clients``` section listed at the top of the example ```${HOME}/.ssh```.
 ...
 * You can now login to the _UI_ named ```{{ groups['user-interface'] | first | regex_replace('^' + ai_jumphost + '\\+','') }}``` with the account as specified in your ```~/.ssh/config```. 
 ...
 * You can also transfer data with ```rsync``` over _SSH_ to copy files to for example your home dir on the cluster with something like the command below.  
 ...
 * If you also added the ```Host *+*+*``` code block from the example ```~/.ssh/config``` you can do tripple hops starting with a jumphost like this:
...
* Q: Why do I get the error ```ControlPath too long```?  
  A: The ```ControlPath ~/.ssh/tmp/%C``` line in your ```${HOME}/.ssh``` file expands to a path that is too long.
...
* Q: Why do I get the error ```nc: getaddrinfo: Name or service not known. ssh_exchange_identification: Connection closed by remote host```?  
  A: Either this server does not exist (anymore) or you have a typo in the name of the server you are trying to connect to.
     Check both the command you typed as well as your ```${HOME}/.ssh``` for typos in server names.
...
* Q: Can I increase the verbosity to debug connection problems?  
  A: Yes try adding ```-vvv``` like this:  
     ...
     If that does not help to figure out what is wrong please [contact the helpdesk](../contact/) and include:  
      ...
      3. A copy of your ```${HOME}/.ssh``` file.  
