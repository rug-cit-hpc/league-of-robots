...
if [[ -z "${APPTAINER_BINDPATH:-}" ]]; then
  export APPTAINER_BINDPATH="{{ slurm_local_scratch_dir | default('/local') }}/${SLURM_JOB_ID}/"
else
  export APPTAINER_BINDPATH="${APPTAINER_BINDPATH},{{ slurm_local_scratch_dir | default('/local') }}/${SLURM_JOB_ID}/"
fi
```
